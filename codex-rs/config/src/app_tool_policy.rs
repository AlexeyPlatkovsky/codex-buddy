...
/// Metadata used to evaluate one app tool.
...
impl<'a> AppToolPolicyEvaluator<'a> {
    ...
    /// Returns whether configuration explicitly affects the app's source state.
    pub fn has_app_enabled_policy(&self, connector_id: &str) -> bool {
        self.apps_config.as_ref().is_some_and(|apps_config| {
